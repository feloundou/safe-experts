...
# from behavioral_cloning.clone_utils import GailClone
from ppo_algos import MLPActorCritic, Discriminator, VDB
...
config_name_list = ['marigold']

# gail_clone_pi = DistilledGaussianActor(env.observation_space.shape[0], env.action_space.shape[0],
#                                             activation=nn.LeakyReLU, n_experts=2, **ac_kwargs)
#
# # Optimizer and criterion for distilled clone
# distilled_pi_optimizer = AdaBelief(gail_clone_pi.parameters(), betas=(0.9, 0.999), eps=1e-16)
# distilled_criterion = nn.MSELoss()

marigold_clone_gail = GailClone(config_name_list=config_name_list,
                                record_samples=True,
                                clone_epochs=100,
                                seed=0,
...
marigold_clone_gail.train_clone(lambda: gym.make(ENV_NAME),
                                actor_critic=MLPActorCritic,
                                discrim = Discriminator,
                                ac_kwargs=dict(hidden_sizes=[hid_size] * n_layers),
                                discrim_kwargs=dict(hidden_sizes=[hid_size]*n_layers),
                                gamma=0.99,
                                lam=0.98,
...
# # VAIL
# ############
# marigold_clone_vail = VailClone(config_name_list=config_name_list,
#                                 record_samples=True,
#                                 clone_epochs=100,
#                                 seed=0,
#                                 expert_episodes=1000,
...
#
#
# marigold_clone_vail.train_clone(lambda: gym.make(ENV_NAME),
#                                 actor_critic=MLPActorCritic,
#                                 vdb=VDB,
#                                 ac_kwargs=dict(hidden_sizes=[hid_size] * n_layers),
#                                 discrim_kwargs=dict(hidden_sizes=[hid_size]*n_layers),
...
# marigold_clone_distill.distilltrain(env=env, train_iters=200, batch_size=100,
